...
- **`--sandbox` is the real thing, and it is opt-in.** With the optional `@anthropic-ai/sandbox-runtime` dependency, `--sandbox` wraps the target in an OS sandbox; measured on macOS, the self-test then passes and the posture rises to `namespace`. WITHOUT it the posture is `none` and absolute paths still resolve as the invoking user — so do not describe the default path as isolation. Two things in that profile are load-bearing and non-obvious, both measured: `denyRead` is REQUIRED because SRT's default restricts writes and network but NOT reads, and `allowUnixSockets` must name the sandbox temp dir as a BARE PATH because a glob silently fails to match for bind, which kills any Node toolchain target with EPERM before it can be introspected.
